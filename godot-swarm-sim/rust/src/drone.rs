...
use quad_flight_control::rc_input::{self, ChannelMap};
use quad_flight_control::{RcInput, RAD_TO_DEG};
...
impl IRigidBody3D for DroneController {
    ...
    fn physics_process(&mut self, dt: f64) {
        ...
        // BodyTruth is rad / rad-s now (post quad-flight-control extraction);
        // godot get_angular_velocity is rad/s, so no scaling — just the
        // axis remap + sign flip that matches the FC's convention.
        let truth = BodyTruth {
            gyro: [-body_ang_vel.z, -body_ang_vel.x, -body_ang_vel.y],
            attitude: [0.0; 3],
        };
        ...
        if whole > self.last_logged_second {
            ...
            godot_print!(
                ...
                input.truth.gyro[0] * RAD_TO_DEG, input.truth.gyro[1] * RAD_TO_DEG, input.truth.gyro[2] * RAD_TO_DEG
            );
        }
